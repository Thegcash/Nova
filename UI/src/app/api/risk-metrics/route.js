...
export async function GET() {
  try {
    ...
    const last30 = await query("select count(*)::int as c from exposures_daily where date >= current_date - interval '30 days'");
    return NextResponse.json({ totals: {
      exposures: exp.rows[0]?.c || 0, losses: los.rows[0]?.c || 0, exposures_last_30d: last30.rows[0]?.c || 0
    }, series: [] });
  } catch (e) {
